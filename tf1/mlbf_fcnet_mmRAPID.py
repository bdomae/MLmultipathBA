...
class mlbf_fcnet:
    ...
    def train(self, dataset, PLOT_PROGRESS=True):
        ...
        #     outputs = layers.Dense(NUM_CLASSES, name='predictions')(drp2)
            ...
            inputs = keras.Input(shape=(self.NUM_MEAS,), name='SparseRSSI')
            fc1 = layers.Dense(fc_dims[0], activation='relu', name='dense_1')(inputs)
            bn1 = layers.BatchNormalization()(fc1)
            #drp1 = layers.Dropout(dropout_rate)(bn1)
            ...
            bn2 = layers.BatchNormalization()(fc2)
            #drp2 = layers.Dropout(dropout_rate)(bn2)
            outputs = layers.Dense(self.NUM_CLASSES, name='predictions')(bn2)
            ...
            # Train the NN
            loss_hist = model.fit(dataset.train_data_dict[train_dict_label][pn_i], dataset.train_classes_dict[train_dict_label][pn_i], epochs=self.NUM_EPOCHS, batch_size=self.BATCH_SIZE)
    ...
    def test(self, dataset, 
        ...
        #         SNR_i = DATA_SNR[snr_ind]
        #         print("{} dB SNR --".format(SNR_i))
                test_predictions = self.all_models[pn_i].predict(dataset.test_data_dict[snr_tag][pn_i])
                ...
                test_loss, test_acc = self.all_models[pn_i].evaluate(dataset.test_data_dict[snr_tag][pn_i],  dataset.test_classes_dict[snr_tag][pn_i], verbose=2)
                ...
                all_curMeas_test_confusion[pn_i, snr_ind, 0:test_confusion.shape[0], 0:test_confusion.shape[1]] = test_confusion
                all_curMeas_pe[pn_i, snr_ind, 0:len(pe)] = pe
